...
static bool sb16_set_sample_rate(u32 rate_hz) {
    ...
    /* Program DSP sample rate */
    dsp_write(DSP_CMD_SET_RATE);
    dsp_write(static_cast<u8>((rate_hz >> 8) & 0xFF));
    dsp_write(static_cast<u8>(rate_hz & 0xFF));
    return true;
}
...
static bool sb16_play(const u8* samples, u32 length, sound_format fmt) {
    ...
    if (fmt == sound_format::signed_16) {
        ...
        u32 sample_count = (transfer / 2) - 1;

        /* DSP command: 16-bit single-cycle output */
        dsp_write(DSP_CMD_PLAY_16BIT | 0x00);  /* D/A, single-cycle */
        dsp_write(DSP_MODE_SIGNED | DSP_MODE_MONO);
        ...
        dsp_write(DSP_CMD_PLAY_8BIT | 0x00);
