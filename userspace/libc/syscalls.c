 ...
 * Heap — _sbrk
 *
 * We request one large arena from the kernel's vk_malloc and hand
...
void* _sbrk(ptrdiff_t incr)
{
    ...
    if (_heap_start == 0) {
        _heap_start = (char*)vk_malloc(VK_HEAP_INITIAL_SIZE);
        if (!_heap_start) {
            errno = ENOMEM;
            return (void*)-1;
        }
    ...
    vk_usize grow = (vk_usize)incr > VK_HEAP_GROW_SIZE
                        ? (vk_usize)incr
                        : VK_HEAP_GROW_SIZE;
    char* extra = (char*)vk_malloc(grow);
    if (!extra) {
        errno = ENOMEM;
        return (void*)-1;
    }
...
int _open(const char* path, int flags, int mode)
{
    ...
    vk_file_handle_t h = vk_get_api()->vk_file_open(path, vk_mode);
    if (h == 0) {
        errno = ENOENT;
        return -1;
    }
...
int _close(int fd)
{
    ...
    int rc = vk_get_api()->vk_file_close(_fd_table[fd].handle);
    _fd_free(fd);
    return rc;
}
...
int _read(int fd, char* buf, int len)
{
    ...
    vk_usize n = vk_get_api()->vk_file_read_handle(
                        _fd_table[fd].handle, buf, (vk_usize)len);
    return (int)n;
}
...
int _write(int fd, const char* buf, int len)
{
    ...
    vk_usize n = vk_get_api()->vk_file_write_handle(
                        _fd_table[fd].handle, buf, (vk_usize)len);
    return (int)n;
}
...
int _lseek(int fd, int offset, int whence)
{
    ...
    int rc = vk_get_api()->vk_file_seek(
                    _fd_table[fd].handle, (vk_i64)offset, whence);
    if (rc != 0) {
        errno = EINVAL;
        return -1;
    }

    vk_i64 pos = vk_get_api()->vk_file_tell(_fd_table[fd].handle);
    return (int)pos;
}
...
int _stat(const char* path, struct stat* st)
{
    ...
    if (vk_file_exists(path)) {
        st->st_mode = S_IFREG;
        st->st_size = (off_t)vk_file_size(path);
        return 0;
    }
